...
std::pair<std::vector<cv::Point>, std::vector<cv::Point>> find_matching_points(cv::Mat img1, cv::Mat img2, std::vector<cv::Point> coord_array, std::vector<cv::Vec3d> epiline_array, int window_size=5, float l_ratio = 0.8, int normType=cv::NORM_L1) {
...
int main() {
    ...
    // Define the line parameters (2x - 3y + 5 = 0)
    double a = 2.0;
    double b = -3.0;
    double c = 5.0;

    // Calculate the ROI coordinates (adjust as needed)
    int roiWidth = 200; // Width of the ROI
    int roiHeight = 150; // Height of the ROI
    int roiX = 100; // X-coordinate of the top-left corner of ROI
    int roiY = 100; // Y-coordinate of the top-left corner of ROI
    ...
    // Detect keypoints and compute descriptors within the ROI
    std::vector<cv::KeyPoint> keypoints;
    cv::Mat descriptors;
    sift->detectAndCompute(roiImage, cv::noArray(), keypoints, descriptors);

    // Filter keypoints based on their proximity to the line
    double distanceThreshold = 10.0; // Adjust as needed
    std::vector<cv::KeyPoint> keypointsInRegion;
    for (const auto& kp : keypoints) {
        double distance = std::abs(a * kp.pt.x + b * kp.pt.y + c) / std::sqrt(a * a + b * b);
        if (distance < distanceThreshold) {
            keypointsInRegion.push_back(kp);
        }
    }

    // Draw keypoints on the ROI image
    cv::Mat roiImageWithKeypoints;
    cv::drawKeypoints(roiImage, keypointsInRegion, roiImageWithKeypoints);

    // Display the result
    cv::imshow("Keypoints Around Line (ROI)", roiImageWithKeypoints);
    cv::waitKey(0);
